...
// Format:  header(4 bytes)  + MAX_REC * Record(12 bytes)
//   header: uint16 head, uint16 count
//   Record: uint32 ts + float level_pct + float volume_liters
...
inline void storageWrite(const SensorData &s) {
  ...
  HistRecord rec = { s.timestamp, s.level_pct, s.volume_liters };
